---
title: "5 Circom Security Pitfalls That Can Break Your Proofs"
slug: "circom-pitfalls"
category: "blog"
description: "Common security vulnerabilities in Circom circuits and how to avoid them, including constraint issues, dependency graphs, and edge cases"
---
...
Here the value `n * 512 + rem` is constrained to `total_bits + 576` but the individual values of n and rem still remain unconstrained. Hence, the circuit dependence graph is broken. Drawing a CDG helps find out the blind spots and vulnerabilities as in this case a malicious prover can set `rem` to `total_bits + 576` and `n` to 0, and generate fake proofs.
...
In the above circuit, there's no constraint to check if $1 + \text{in}[0] \neq 0$ and $\text{in}[1] \neq 0$. When $1 + \text{in}[0]$ or $\text{in}[1]$ is set to 0, their corresponding multipliers in the same terms, namely `out[1]` and `out[0]` become underconstrained. Attackers can construct an exploit to bypass the restrictions on circuit outputs. This was fixed by adding a constraint to check if the divisor is non zero using the `IsZero()` template.

Here's another edge case from Electisec's review of Spartan Ecdsa:

In the `Secp256k1AddComplete()` circuit of Spartan ECDSA, an edge case arises when adding two distinct points P and Q such that $x_P \neq x_Q$ but $y_P + y_Q = 1$.

Here:
- $x_P, y_P$ - x and y coordinates of point P
- $x_Q, y_Q$ - x and y coordinates of point Q
...
- Practical Security Analysis of Zero-Knowledge Proof Circuits - [https://eprint.iacr.org/2023/190.pdf](https://eprint.iacr.org/2023/190.pdf)
- Certifying Zero-Knowledge Circuits with Refinement Types - [https://eprint.iacr.org/2023/547.pdf](https://eprint.iacr.org/2023/547.pdf)
- Automated Detection of Under-Constrained Circuits in Zero-Knowledge Proofs - [https://dl.acm.org/doi/pdf/10.1145/3591282](https://dl.acm.org/doi/pdf/10.1145/3591282)
- ZK bug Tracker by 0xPARC - [https://github.com/0xPARC/zk-bug-tracker](https://github.com/0xPARC/zk-bug-tracker)
- ZK Security Reports - [https://github.com/nullity00/zk-security-reviews](https://github.com/nullity00/zk-security-reviews)

Here are some of the tools used to debug circom circuits:

- Picus - [https://github.com/Veridise/Picus](https://github.com/Veridise/Picus)
- circomspect - [https://github.com/trailofbits/circomspect](https://github.com/trailofbits/circomspect)
