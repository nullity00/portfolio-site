...
1. **Validator A** generates polynomial $f_a(x) = a_0 + a_1x + a_2x^2 + a_3x^3 + a_4x^4$ and:
...
2. **Validator B** generates polynomial $f_b(x) = b_0 + b_1x + b_2x^2 + b_3x^3 + b_4x^4$ and:
...
Where:
- $comm_A$ = Validator A's public commitment to their final share
...
$$
comm_A = F_a(1) + F_b(1) + F_c(1) + F_d(1) + F_e(1) + F_f(1) + F_g(1)
$$
...
4. **Polynomial degree**: Sender's polynomial has correct degree (≤ t-1)
